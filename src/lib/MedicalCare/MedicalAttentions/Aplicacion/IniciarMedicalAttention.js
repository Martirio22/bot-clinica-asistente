  ...
  async ejecutar(data, userIdFromToken) {
    ...
    if (!doctor || doctor.userId !== userIdFromToken) {
      throw new ValidationError(
        "No puedes iniciar una atención de una cita que no te pertenece"
      );
    }
    ...
    if (existing) {
      throw new ConflictError("Ya existe una atención iniciada para esta cita");
    }

    const statusEnProcesoId = await this.attentionRepo.findStatusByCode("EN_PROCESO");
    const appointmentAtendidaId = await this.appointmentRepo.findStatusByCode("ATENDIDA");

    if (!statusEnProcesoId) {
      throw new NotFoundError("Estado 'EN_PROCESO' no configurado");
    }

    if (!appointmentAtendidaId) {
      throw new NotFoundError("Estado 'ATENDIDA' no configurado");
    }
