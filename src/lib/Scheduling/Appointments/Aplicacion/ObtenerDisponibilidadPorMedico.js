class ObtenerDisponibilidadMedico {
  ...
  async ejecutar(doctorId, fecha) {
    ...
    const [citas, bloqueos] = await Promise.all([
      ...
      this.blockingRepo.findAllByDoctorAndDate(doctorId, fecha)
    ]);
    ...
    while (current.getTime() + duracionMs <= end.getTime()) {
      ...
      if (!this._hayConflicto(slotInicio, slotFin, citas, bloqueos)) {
        slots.push({
          inicio: new Date(slotInicio).toLocaleString("sv-SE", {
            timeZone: "America/Guayaquil"
          }),
          fin: new Date(slotFin).toLocaleString("sv-SE", {
            timeZone: "America/Guayaquil"
          })
        });
      }
    ...
    return {
      doctorId,
      fecha,
      duracionCita: doctor.appointmentDurationMinutes,
      sucursalId: horarioHoy.branchId,
      consultorioId: horarioHoy.officeId,
      slots
    };
  }
